...
function media(row:Row){const result:string[]=[];const add=(value:unknown)=>{if(value===null||value===undefined)return;if(typeof value==="string"){const item=value.trim();if(!item)return;if(item.startsWith("[")){try{const parsed=JSON.parse(item);if(Array.isArray(parsed)){parsed.forEach(add);return;}}catch{}}if(/^https?:\/\//i.test(item)||item.startsWith("/"))result.push(item);return;}if(Array.isArray(value)){value.forEach(add);return;}if(typeof value!=="object")return;const nested=record(value);const next=nested.url??nested.imageUrl??nested.image_url??nested.path;if(next!==undefined&&next!==value)add(next);};["images","image_urls","imageUrls","media","image_url","imageUrl","cover_image","coverImage","image"].forEach((key)=>add(row[key]));return Array.from(new Set(result));}
