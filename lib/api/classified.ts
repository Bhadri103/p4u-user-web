...
function mediaList(row: UnknownRow): string[] {
  const output:string[]=[]; const add=(value:unknown)=>{ if(typeof value==="string"){const v=value.trim();if(!v)return;if(v.startsWith("[")){try{const parsed=JSON.parse(v);if(Array.isArray(parsed)){parsed.forEach(add);return;}}catch{}}if(/^https?:\/\//i.test(v)||v.startsWith("/"))output.push(v);}else if(Array.isArray(value))value.forEach(add);else{const item=record(value);add(item.url??item.imageUrl??item.image_url??item.mediaUrl??item.path);} };
