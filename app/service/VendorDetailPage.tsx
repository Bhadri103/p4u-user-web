...
const IC = {
  Star:      ({ fill="#B8E3F7",size=13 }:{ fill?:string;size?:number })=><svg width={size} height={size} viewBox="0 0 24 24" fill={fill} stroke={fill} strokeWidth="1"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>,
  ...
  Heart:     ({ filled }:{ filled:boolean })=><svg width="14" height="14" fill={filled?"#ef4444":"none"} stroke={filled?"#ef4444":"#5D757A"} strokeWidth="2" viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/></svg>,
  ...
  Check:     ()=><svg width="18" height="18" fill="none" viewBox="0 0 24 24"><path d="M22 11.08V12a10 10 0 11-5.93-9.14" stroke="#89CFF0" strokeWidth="2"/><polyline points="22 4 12 14.01 9 11.01" stroke="#89CFF0" strokeWidth="2"/></svg>,
...
function Checkbox({ checked,onChange,accent=TEAL }:{ checked:boolean;onChange:()=>void;accent?:string }) {
  return (
    <div onClick={onChange} style={{ width:15,height:15,borderRadius:3,border:`2px solid ${checked?accent:"#D7E7F5"}`,background:checked?accent:"#fff",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,cursor:"pointer",transition:"all .15s" }}>
...
function SlideContent({ slide, vendorName }:{ slide:BannerSlide; vendorName:string }) {
  return (
    <> 
      ...
      <div style={{ position:"absolute", top:14, right:14, background:slide.accent, color:"#fff", fontSize:11, fontWeight: 600,
      ...
      <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", justifyContent:"center", padding:"0 32px" }}>
        ...
        <div style={{ fontSize:30, fontWeight: 600, color:"#fff", lineHeight:1.05, marginBottom:14,
        ...
        <div style={{ display:"inline-flex", alignItems:"center", gap:8, background:`${slide.accent}22`,
          ...
          <IC.Zap />
          <span style={{ fontSize:13, fontWeight: 600, color:"#fff" }}>{slide.subtitle}</span>
        </div>
      </div>
    </>
  );
...
function VendorInfoCard({
  ...
  return (
    <div style={{ background:"#fff",borderRadius:16,padding:"16px 20px",marginBottom:16,boxShadow:"0 2px 12px rgba(0,0,0,0.08)",border:"1px solid #F7FBFF" }}>
      <div style={{ display:"flex",gap:16,alignItems:"flex-start",flexWrap:"wrap" }}>
         <VendorLogo vendor={vendor}/>
     <div style={{ flex:1,minWidth:220 }}>
          <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:3 }}>
            <h2 style={{ fontSize:18,fontWeight: 600,color:"#202124",margin:0 }}>{vendor.name}</h2>
            {vendor.verified&&<IC.Check/>}
          </div>
          <p style={{ fontSize:12,color:"#5D757A",margin:"0 0 10px",lineHeight:1.55,maxWidth:480 }}>{vendor.description}</p>
           <div style={{ display:"flex",flexWrap:"wrap",gap:0 }}>
            {[
              ...
              <div key={item.label} style={{ display:"flex",alignItems:"stretch",paddingRight:i<arr.length-1?16:0 }}>
                <div>
                  <div style={{ fontSize:10,color:"#5D757A",marginBottom:2 }}>{item.label}</div>
                  {item.value
                    ?<div style={{ fontSize:12,fontWeight: 600,color:"#202124",whiteSpace:"nowrap" }}>{item.value}</div>
                    :<div style={{ display:"flex",alignItems:"center",gap:4 }}><IC.Star size={12}/><span style={{ fontSize:12,fontWeight: 600,color:"#202124" }}>{vendor.rating}/5</span><span style={{ fontSize:10,color:"#5D757A" }}>({vendor.totalRatings})</span></div>}
                </div>
                {i<arr.length-1&&<div style={{ width:1,background:"#D7E7F5",margin:"0 12px",alignSelf:"stretch" }}/>}
              </div>
            ))}
          </div>
           <div style={{ display:"flex",gap:8,marginTop:12 }}>
            <button
              type="button"
              onClick={onBookNow}
              style={{ display:"flex",alignItems:"center",gap:6,padding:"8px 20px",background:TEAL_GRAD,border:"none",borderRadius:20,color:"#fff",fontSize:12,fontWeight: 600,cursor:onBookNow?"pointer":"default",boxShadow:"0 4px 12px rgba(13,148,136,0.35)",opacity:onBookNow?1:0.85 }}
            >
        ...
        <div style={{ flexShrink:0,minWidth:200,borderLeft:"1px solid #F7FBFF",paddingLeft:20 }}>
          <div style={{ fontSize:12,fontWeight: 600,color:"#202124",marginBottom:10 }}>Contact Details</div>
          <div style={{ display:"flex",flexDirection:"column",gap:8 }}>
            <div style={{ display:"flex",alignItems:"center",gap:8,fontSize:12 }}><span style={{ color:"#89CFF0" }}><IC.Phone/></span><span style={{ color:"#202124" }}>{vendor.phone}</span></div>
            <div style={{ display:"flex",alignItems:"center",gap:8,fontSize:12 }}><span style={{ color:"#89CFF0" }}><IC.Mail/></span><span style={{ color:"#202124",fontSize:11 }}>{vendor.email}</span></div>
            <div style={{ display:"flex",alignItems:"flex-start",gap:8,fontSize:12 }}><span style={{ color:"#ef4444",marginTop:1 }}><IC.MapPin/></span><span style={{ color:"#202124",fontSize:11,lineHeight:1.4 }}>{vendor.address}</span></div>
          </div>
          <div style={{ display:"flex",gap:8,marginTop:12 }}>
            <button style={{ display:"flex",alignItems:"center",gap:5,padding:"7px 14px",background:TEAL_GRAD,border:"none",borderRadius:8,color:"#fff",fontSize:11,fontWeight: 600,cursor:"pointer" }}><IC.Phone/> Send Message</button>
            <button style={{ display:"flex",alignItems:"center",gap:5,padding:"7px 14px",background:"#89CFF0",border:"none",borderRadius:8,color:"#fff",fontSize:11,fontWeight: 600,cursor:"pointer" }}><IC.MapPin/> Directions</button>
          </div>
        </div>
      </div>
    </div>
...
function ProductCard({ product, onBook }: ProductCardProps) {
  ...
  return (
    <div
      style={{ background:"#fff",borderRadius:16,overflow:"hidden",boxShadow:"0 2px 12px rgba(0,0,0,0.08)",border:"1px solid #F7FBFF",display:"flex",flexDirection:"column",cursor:"pointer",transition:"transform .2s,box-shadow .2s" }}
    ...
    > 
      <div style={{ position:"relative",height:155,background:"#FFFFFF",flexShrink:0,overflow:"hidden" }}>
        {imgOk && product.image ? (
        <img
          src={product.image}
          alt={product.name}
        ...
        <div style={{ width:"100%",height:"100%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,color:"#5D757A" }}>No image</div>
        )} 
        {product.badge && (
          <div style={{ position:"absolute",top:8,left:8,background:"rgba(255,255,255,0.93)",color:TEAL,fontSize:9,fontWeight: 600,padding:"3px 8px",borderRadius:12,border:`1.5px solid ${TEAL}`,backdropFilter:"blur(4px)",zIndex:2 }}>
            {product.badge}
          </div>
        )} 
        <button
      ...
      <div style={{ padding:"10px 12px 12px",display:"flex",flexDirection:"column",flex:1 }}>
        <h3 style={{ fontSize:12,fontWeight: 600,color:"#202124",margin:"0 0 3px",lineHeight:1.3,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden" }}>
          {product.name}
        </h3>
        <div style={{ fontSize:10,color:"#5D757A",marginBottom:6,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis" }}>
          {product.brand}
        </div>
 
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6 }}>
          <div>
            <span style={{ fontSize:13,fontWeight: 600,color:"#202124" }}>₹{product.price}</span>
            {product.originalPrice && product.originalPrice > product.price && (
              <span style={{ fontSize:10,color:"#5D757A",textDecoration:"line-through",marginLeft:4 }}>₹{product.originalPrice}</span>
            )}
          </div>
          <div style={{ display:"flex",alignItems:"center",gap:3,background:"#fffbeb",padding:"2px 7px",borderRadius:20 }}>
            <IC.Star size={10}/>
            <span style={{ fontSize:10,fontWeight: 600,color:"#d97706" }}>{product.rating}</span>
            <span style={{ fontSize:9,color:"#5D757A" }}>({product.reviews})</span>
          </div>
        </div> 
        <div style={{ fontSize:10,color:"#5D757A",marginBottom:8,display:"flex",alignItems:"center",gap:4 }}>
          <IC.Clock/>
          <span>Starts at ₹{product.price} • {product.duration}</span>
        </div>
 
        <div style={{ borderTop:"1px solid #F7FBFF",paddingTop:8,display:"flex",gap:6,alignItems:"center" }}>
          <button
            onClick={handleBook}
            style={{
              flex: 1,
              height: 30,
              border: "1.5px solid #D7E7F5",
              borderRadius: 10,
              background: "#fff",
              color: "#202124",
              fontSize: 11,
...
function RatingSection({ vendor }:{ vendor:Vendor }) {
  return (
    <div style={{ background:"#FFFFFF",borderRadius:14,padding:"18px 20px",marginTop:16 }}>
      <h4 style={{ fontSize:14,fontWeight: 600,color:"#202124",margin:"0 0 14px" }}>Ratings & Reviews</h4>
      <div style={{ display:"flex",gap:28,alignItems:"center" }}>
        <div style={{ textAlign:"center",flexShrink:0 }}>
          <div style={{ fontSize:52,fontWeight: 600,color:"#202124",lineHeight:1 }}>{(Number.isFinite(vendor.rating) ? vendor.rating : 0).toFixed(1)}</div>
          <div style={{ display:"flex",justifyContent:"center",gap:2,margin:"6px 0 4px" }}>{[1,2,3,4,5].map(n=><IC.Star key={n} size={15} fill={n<=Math.floor(Number.isFinite(vendor.rating)?vendor.rating:0)?"#B8E3F7":"#D7E7F5"}/>)}</div>
          <div style={{ fontSize:11,color:"#5D757A" }}>{vendor.totalRatings} {vendor.totalRatings === 1 ? "rating" : "ratings"}</div>
        </div>
        <p style={{ flex:1,fontSize:12,color:"#5D757A",margin:0,lineHeight:1.5 }}>
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(170px,1fr))", gap: 14 }}>
      {Array.from({ length: 8 }).map((_, i) => (
        <div key={i} style={{ height: 280, borderRadius: 16, background: "#FFFFFF" }} />
      ))}
    </div>
  );
}
...
export default function VendorDetailPage({
    ...
    return (
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: 16 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
          ...
          <span style={{ fontSize: 12, color: "#5D757A" }}>Loading vendor…</span>
        </div>
        <div style={{ height: 200, borderRadius: 16, background: "#FFFFFF", marginBottom: 16 }} />
        <div style={{ height: 140, borderRadius: 16, background: "#FFFFFF", marginBottom: 16 }} />
        <ProductGridSkeleton />
      </div>
    );
  }
  ...
  if (!vendor) {
    return (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 12, fontFamily: "'Segoe UI',system-ui,sans-serif" }}>
        <p style={{ color: "#5D757A", fontSize: 14 }}>Vendor not found.</p>
    ...
    <div> 
      <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:14,flexWrap:"wrap" }}>
        <div style={{ display:"flex",gap:6,overflowX:"auto",flex:1,paddingBottom:2 }}>
          {chips.map(chip=>(
            <button key={chip} onClick={()=>setCategoryChip(chip)} style={{ padding:"5px 13px",borderRadius:20,fontSize:11,fontWeight:600,border:`1px solid ${categoryChip===chip?TEAL:"#D7E7F5"}`,background:categoryChip===chip?TEAL_GRAD:"#fff",color:categoryChip===chip?"#fff":"#5D757A",cursor:"pointer",whiteSpace:"nowrap",flexShrink:0,transition:"all .15s" }}>{chip}</button>
          ))}
        </div>
        <div style={{ display:"flex",alignItems:"center",border:"1px solid #D7E7F5",borderRadius:9,overflow:"hidden",background:"#fff" }}>
        ...
        <button onClick={()=>setFilterDrawerOpen(true)} style={{ display:"flex",alignItems:"center",gap:5,padding:"6px 12px",border:"1px solid #D7E7F5",borderRadius:8,background:"#fff",fontSize:11,fontWeight:600,color:"#202124",cursor:"pointer" }}>
          <IC.Filter/> Filter {activeFilters.length>0&&<span style={{ background:TEAL_GRAD,color:"#fff",borderRadius:"50%",width:16,height:16,fontSize:10,display:"flex",alignItems:"center",justifyContent:"center",fontWeight: 600 }}>{activeFilters.length}</span>}
        </button>
      </div>
      ...
      {loadingProducts ? (
        <div>
          <p style={{ fontSize: 12, color: "#5D757A", marginBottom: 12 }}>Loading bookable items…</p>
          <ProductGridSkeleton />
        </div>
      ) : filtered.length === 0 ? (
        <div style={{ padding:"60px 20px",textAlign:"center",color:"#5D757A",fontSize:14 }}>No services found. Try adjusting your filters.</div>
      ) : (
        <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(170px,1fr))",gap:14 }}>
          {filtered.map((p) => (
            <ProductCard
  ...
  const renderAbout = ()=>(
    <div>
      <h4 style={{ fontSize:15,fontWeight: 600,color:"#202124",margin:"0 0 12px" }}>About {vendor.name}</h4>
      <p style={{ fontSize:13,color:"#5D757A",lineHeight:1.7,marginBottom:16 }}>{vendor.description}</p>
      <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(170px,1fr))",gap:10 }}>
        {[
          ...
          <div key={item.label} style={{ background:"#FFFFFF",borderRadius:10,padding:"12px 14px" }}>
            <div style={{ fontSize:10,color:"#5D757A",marginBottom:4 }}>{item.label}</div>
  ...
  const renderGallery = ()=>(
    <div style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12 }}>
      {vendor.products.length === 0 ? (
        <p style={{ gridColumn: "1 / -1", textAlign: "center", color: "#5D757A", fontSize: 14 }}>No gallery images yet.</p>
      ) : (
      vendor.products.slice(0,9).map((p,i)=>(
        <div key={i} style={{ borderRadius:12,overflow:"hidden",aspectRatio:"1",background:"#D7E7F5" }}>
          {p.image ? (
            <img src={p.image} alt={p.name} style={{ width:"100%",height:"100%",objectFit:"cover" }}/>
          ) : (
            <div style={{ width:"100%",height:"100%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,color:"#5D757A" }}>No image</div>
          )}
        </div>
      )))}
    </div>
  ...
  return (
    <div >
      <div style={{ maxWidth:1200,margin:"0 auto",padding:"16px" }}> 
        <div style={{ display:"flex",alignItems:"center",gap:6,fontSize:12,color:"#5D757A",marginBottom:16 }}>
          <span style={{ cursor:"pointer",transition:"color .15s" }} onClick={onBack} onMouseEnter={e=>(e.currentTarget as HTMLSpanElement).style.color="#202124"} onMouseLeave={e=>(e.currentTarget as HTMLSpanElement).style.color="#5D757A"}>Shop</span>
          <span style={{ color:"#D7E7F5" }}>›</span>
          <span style={{ cursor:"pointer" }} onClick={onBack}>{vendor.category}</span>
          <span style={{ color:"#D7E7F5" }}>›</span>
          <span style={{ color:"#202124",fontWeight:600 }}>{vendor.name}</span>
        </div> 
        ...
        /> 
        <div style={{ background:"#fff",borderRadius:16,padding:"16px 20px",boxShadow:"0 2px 12px rgba(0,0,0,0.06)",border:"1px solid #F7FBFF" }}>
          {/* Tab bar */}
          <div style={{ display:"flex",gap:0,borderBottom:"2px solid #F7FBFF",marginBottom:18 }}>
            {vendor.tabs.map((tab,i)=>(
              <button key={tab} onClick={()=>setActiveTab(i)} style={{ padding:"10px 24px",fontSize:13,fontWeight:activeTab===i?700:500,color:activeTab===i?TEAL:"#5D757A",border:"none",background:"transparent",cursor:"pointer",borderBottom:activeTab===i?`2px solid ${TEAL}`:"2px solid transparent",marginBottom:-2,transition:"all .15s",display:"flex",alignItems:"center",gap:6 }}>
                {tabIcons[i]}{tab}
              </button>
            ))}
          </div> 
          {activeTab===0&&<h3 style={{ fontSize:14,fontWeight: 600,color:"#202124",margin:"0 0 14px" }}>Service List</h3>}
        ...
        <div style={{ position:"fixed",inset:0,zIndex:400 }}>
          ...
          <div style={{ position:"absolute",right:0,top:0,bottom:0,width:280,background:"#fff",padding:20,overflowY:"auto",zIndex:1,boxShadow:"-4px 0 24px rgba(0,0,0,0.15)" }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18 }}>
              <span style={{ fontWeight: 600,fontSize:15,color:"#202124" }}>Filters</span>
              <button onClick={()=>setFilterDrawerOpen(false)} style={{ background:"none",border:"none",cursor:"pointer" }}><IC.X size={18}/></button>
            </div> 
            <div style={{ marginBottom:20 }}>
              <div style={{ fontSize:11,fontWeight: 600,color:"#202124",marginBottom:10,textTransform:"uppercase",letterSpacing:"0.06em" }}>Brands</div>
              {brands.map(b=>(
                <div key={b} onClick={()=>toggleBrand(b)} style={{ display:"flex",alignItems:"center",gap:10,padding:"7px 0",cursor:"pointer" }}>
                  ...
                  <span style={{ fontSize:12,color:selectedBrands.includes(b)?TEAL:"#5D757A",fontWeight:selectedBrands.includes(b)?600:400 }}>{b}</span>
                </div>
              ))}
            </div> 
            <div style={{ marginBottom:20 }}>
              <div style={{ fontSize:11,fontWeight: 600,color:"#202124",marginBottom:10,textTransform:"uppercase",letterSpacing:"0.06em" }}>Rating</div>
              {[4.5,4.0,3.5].map(r=>(
                <div key={r} onClick={()=>setRatingFilter(ratingFilter===r?null:r)} style={{ display:"flex",alignItems:"center",gap:10,padding:"7px 0",cursor:"pointer" }}>
                  <Checkbox checked={ratingFilter===r} onChange={()=>setRatingFilter(ratingFilter===r?null:r)} accent="#B8E3F7"/>
                  <span style={{ fontSize:12,display:"flex",alignItems:"center",gap:3,color:"#5D757A" }}>
                    {[1,2,3,4].map(n=><IC.Star key={n} size={11} fill={n<=Math.floor(r)?"#B8E3F7":"#D7E7F5"}/>)} {r}+
                  </span>
                </div>
              ))}
            </div>
            ...
            <div style={{ marginBottom:24 }}>
              <div style={{ fontSize:11,fontWeight: 600,color:"#202124",marginBottom:10,textTransform:"uppercase",letterSpacing:"0.06em" }}>Offers</div>
              <div onClick={()=>setOffersOnly(o=>!o)} style={{ display:"flex",alignItems:"center",gap:10,cursor:"pointer" }}>
                ...
                <span style={{ fontSize:12,color:"#5D757A" }}>Show deals only</span>
              </div>
            </div>

            <button onClick={()=>setFilterDrawerOpen(false)} style={{ width:"100%",padding:"11px",background:TEAL_GRAD,border:"none",borderRadius:10,color:"#fff",fontSize:13,fontWeight: 600,cursor:"pointer" }}>Apply Filters</button>
          </div>
        </div>
      )}
    </div>
